...
interface BudgetSetupProps {
  budgets: Budget[];
  onSaveBudgets: (budgets: Omit<Budget, 'spent' | 'remaining' | 'percentage'>[]) => void;
}

export function BudgetSetup({ budgets, onSaveBudgets }: BudgetSetupProps) {
  const [localBudgets, setLocalBudgets] = useState<Omit<Budget, 'spent' | 'remaining' | 'percentage'>[]>(
  ...
  const updateBudgetAmount = (id: string, amount: string) => {
    setLocalBudgets(localBudgets.map(b => 
  ...
  const handleCancel = () => {
    setLocalBudgets(budgets.map(b => ({ id: b.id, category: b.category, amount: b.amount })));
    setNewCategory('');
    setNewAmount('');
    setIsOpen(false);
  };
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Set monthly spending limits for different categories to track your financial goals.
          </DialogDescription>
        </DialogHeader>
              ...
              {localBudgets.length === 0 ? (
                ...
                <div className="space-y-3">
                  {localBudgets.map((budget) => (
                    <div key={budget.id} className="flex items-center gap-3 p-3 border rounded-lg">
                      ...
                      <div className="flex items-center gap-2">
                        <Input
                          ...
                          step="0.01"
                          value={budget.amount}
                          onChange={(e) => updateBudgetAmount(budget.id, e.target.value)}
                          className="w-32"
                        />
                        <Button
                          variant="ghost"
                  ...
                  <div className="pt-2 border-t">
                    <div className="flex justify-between items-center font-semibold">
                      ...
                      <span>{formatCurrency(localBudgets.reduce((sum, b) => sum + b.amount, 0))}</span>
                    </div>
                  </div>
                </div>
              )}
